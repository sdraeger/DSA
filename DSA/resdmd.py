...
def _validate_control_input(model, control_data, data) -> None:
    ...
    if control_data is not None:
        ...
        ctrl_arr = (
            control_data.cpu().numpy()
            if hasattr(control_data, "cpu")
            else np.asarray(control_data)
        )
        ...
        if data_arr.ndim == 2:
            ...
            if (
                data_arr.shape[0] != ctrl_arr.shape[0]
                or data_arr.shape[1] != ctrl_arr.shape[1]
            ):
                raise ValueError(
...
def _subtract_control_effects(
    Y: np.ndarray, B: np.ndarray, U: np.ndarray
) -> np.ndarray:
    """
...
def _compute_residuals_from_matrices(
    ...
    for i in range(rank):
        ...
        persistence_numerator = np.dot(g.conj().T, np.dot(YtY - XtY - YtX + XtX, g))
...
def compute_residuals(
    ...
    steps_ahead = (
        dmd.steps_ahead
        if dmd.steps_ahead is not None and hasattr(dmd, "steps_ahead")
        else 1
    )
    device = dmd.device
    ...
    if is_dmdc:
        n_control_delays = (
            dmd.n_control_delays if hasattr(dmd, "n_control_delays") else 1
        )
        H_U = embed_data_DMDc(
            U_data,
            n_delays=n_delays,
...
def compute_residuals_subspace_dmdc(
    ...
    if use_training_latents:
        ...
        if control_data is None:
            raise ValueError(
                "control_data is required for SubspaceDMDc residual computation."
            )
...
class ResidualComputer:
    ...
    def _detect_model_type(self, model) -> str:
        ...
        raise ValueError(
            f"Cannot detect model type for {type(model)}. "
    ...
    def compute(self):
        ...
        if self.model_type == "local_dmd":
            result = compute_residuals(
                self.model, self.test_data, control_data=self.control_data
            )
        elif self.model_type == "subspace_dmdc":
            result = compute_residuals_subspace_dmdc(
                self.model, self.test_data, self.control_data
            )
