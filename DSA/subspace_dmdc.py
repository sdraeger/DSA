...
class SubspaceDMDc(BaseDMD):
    ...
    def _collect_data(self, y_list, u_list, p, f):
        ...
        def hankel_stack(X, start, L):
            ...
            return np.concatenate(
                [X[start + i : start + i + 1, :].T for i in range(L)], axis=0
            )
        ...
        for trial_idx, (Y_trial, U_trial) in enumerate(zip(y_list, u_list)):
            ...
            U_p_trial = np.concatenate(
                [hankel_stack(U_trial, j, p) for j in range(T_trial)], axis=1
            )
            Y_p_trial = np.concatenate(
                [hankel_stack(Y_trial, j, p) for j in range(T_trial)], axis=1
            )
            U_f_trial = np.concatenate(
                [hankel_stack(U_trial, j + p, f) for j in range(T_trial)], axis=1
            )
            Y_f_trial = np.concatenate(
                [hankel_stack(Y_trial, j + p, f) for j in range(T_trial)], axis=1
            )
        ...
        if not valid_trials:
            raise ValueError(
                "No trials have sufficient time points for given number of delays"
            )
    ...
    def subspace_dmdc_multitrial_QR_decomposition(
        ...
        X, X_next, U_mid, Y_curr = self._time_align_valid_trials(
        ...
        noise_covariance, R_hat, Q_hat, S_hat = self._estimate_noise_covariance(
            X_next, A_hat, X, B_hat, U_mid, Y_curr, C_hat
        )
    ...
    def _time_align_valid_trials(
        self, X_hat, u_list, y_list, valid_trials, T_per_trial, p
    ):
    ...
    def _perform_ridge_regression(self, X, X_next, U_mid, n, lamb):
        ...
        if self.use_torch and Z.shape[1] > 100:
            ...
            ridge_term = lamb * torch.eye(
                ZTZ.shape[0], device=self.device, dtype=Z_torch.dtype
            )
    ...
    def _estimate_noise_covariance(self, X_next, A_hat, X, B_hat, U_mid, Y_curr, C_hat):
        ...
        noise_covariance = np.block([[R_hat, S_hat.T], [S_hat, Q_hat]])
        ...
        if isinstance(y, list) and isinstance(u, list):
            ...
            for y_trial, u_trial in zip(y, u):
                if y_trial.ndim == 3 and u_trial.ndim == 3:
                    ...
                    raise ValueError(
                        "Invalid dimension. Only list of (n_trials, n_timepoints, n_features) or (n_timepoints, n_features) arrays are supported."
                    )
        else:
            if y.ndim == 2:
                y_list = [y]
    ...
    def get_training_latent_states(self, return_aligned=True):
        ...
        y_list, u_list = self._convert_to_subspace_dmdc_data_format(
            self.data, self.control_data
        )
    ...
    def project_to_latent(
        self, test_data, control_data, return_aligned=True, method="smooth"
    ):
        """
    ...
    def predict(self, test_data=None, control_data=None, reseed=None):
        ...
        if isinstance(test_data, list):
            ...
            for trial in range(len(test_data)):
                ...
                trial_predictions = [
                    self.kalman.step(
                        y=test_data[trial][t] if t % reseed == 0 else None,
                        u=control_data[trial][t],
                    )[0]
        ...
        if test_data.ndim == 2:
            return np.concatenate(
                [
                    self.kalman.step(
                        y=test_data[t] if t % reseed == 0 else None, u=control_data[t]
                    )[0]
...
class OnlineKalman:
    ...
    def step(self, y=None, u=None, reg_coef=1e-6):
        ...
        p_filtered = self._regularize_and_symmetrize(
            p_pred_reg - K_filtered @ self.C @ p_pred_reg, reg_coef
        )

        x_filtered = (
            x_pred + K_filtered @ (y - self.C @ x_pred)
            if not np.isnan(y).any()
            else x_pred.copy()
        )

        K_pred = (self.S + self.A @ p_pred_reg @ self.C.T) @ np.linalg.pinv(S_innov)
        p_predicted = self._regularize_and_symmetrize(
        ...
        x_predicted = (
            self.A @ x_pred
            + self.B @ u
            + (K_pred @ (y - self.C @ x_pred) if not np.isnan(y).any() else 0)
        )

        self._store_results(
            x_filtered, x_predicted, p_filtered, p_predicted, u, y, K_pred
        )

        return self.y_filtereds[-1], self.x_filtereds[-1]

    def _store_results(
        self, x_filtered, x_predicted, p_filtered, p_predicted, u, y, K_pred
    ):
    ...
    def _predict(self):
        ...
        x_pred = (
            self.x_predicteds[-1] if self.x_predicteds else np.zeros((self.x_dim, 1))
        )
