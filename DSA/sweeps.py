...
import warnings
import torch
from matplotlib.pyplot import savefig, subplots, cm, tight_layout
...
class BaseSweeper(ABC):
    ...
    **model_kwargs
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        if control_data is not None:
            self.train_control, self.test_control, _ = split_train_test(
                control_data, train_frac
            )
        else:
    ...
    def _align_predictions(
        self, pred: np.ndarray, test: np.ndarray
    ...
    def plot(
        ...
        axes[-1].legend(
            title=legend_param, loc="upper right", bbox_to_anchor=(1.4, 1), fontsize=8
        )
...
class DefaultSweeper(BaseSweeper):
    ...
    **kwargs
    ...
    def _is_valid_param_combo(self, p1_val, p2_val) -> bool:
        n_delays = (
            p1_val
            if self.param1_name == "n_delays"
            else (p2_val if self.param2_name == "n_delays" else None)
        )
        rank = (
            p1_val
            if self.param1_name == "rank"
            else (p2_val if self.param2_name == "rank" else None)
        )
    ...
    def make_model(self, p1_val, p2_val):
        kwargs = {
            self.param1_name: p1_val,
            self.param2_name: p2_val,
            **self.model_kwargs,
        }
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.extra_observable_kwargs = extra_observable_kwargs or [
            {} for _ in self.extra_observables
        ]
    ...
    def _parse_param_name(self, param_name: str) -> dict:
        ...
        if len(parts) < 2:
            raise ValueError(
                f"Invalid parameter name: {param_name}. Expected 'component.param' format."
            )
        ...
        if self._is_observable_component(component):
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid extra_obs index in: {param_name}. Expected integer."
                    )
                ...
                raise ValueError(
                    f"Invalid extra_obs format: {param_name}. Use 'extra_obs.N.param' or 'extra_obsN.param'."
                )
            else:
                raise ValueError(
                    f"Invalid extra_obs format: {param_name}. Use 'extra_obs.N.param'."
                )
        else:
            raise ValueError(
                f"Unknown component: {component}. Expected 'obs', 'reg', or 'extra_obs'."
            )
    ...
    def predict(self, model, test_data: np.ndarray, reseed: int) -> np.ndarray:
        ...
        if test_np.ndim == 3:
            if test_u is not None and test_u.ndim == 3:
                return np.stack(
                    [
                        self._predict_single(model, t, u=test_u[i])
    ...
    def _predict_single(
        self, model, data: np.ndarray, u: np.ndarray = None
    ) -> np.ndarray:
...
def sweep_local_dmd(data, n_delays_values, rank_values, **kwargs) -> DefaultSweeper:
    ...
    sweeper = DefaultSweeper(
        data=data, param1_values=n_delays_values, param2_values=rank_values, **kwargs
    )
    sweeper.sweep()
    return sweeper
...
def sweep_pykoopman(
    data, param1_name, param1_values, param2_name, param2_values, **kwargs
) -> PyKoopmanSweeper:
...
def sweep_ranks_delays(
    ...
    **model_kwargs,
    ...
    sweeper = DefaultSweeper(
        data=data,
        param1_values=n_delays,
        param2_values=ranks,
        model_class=model_class,
        control_data=control_data,
        train_frac=train_frac,
        reseed=reseed,
        compute_residuals=return_residuals,
        **model_kwargs,
    )
    sweeper.sweep()
    ...
    if return_residuals:
        result += (
            sweeper.residuals
            if sweeper.residuals is not None
            else np.full_like(sweeper.aics, np.nan),
        )
    return result
