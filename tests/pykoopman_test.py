...
def compute_manual_embedding(data, n_delays, rank):
    ...
    k_manual = Koopman(observables=Identity(), regressor=pDMD(svd_rank=rank))
...
class TestKoopman3DData:
    ...
    def test_3d_timedelay_vs_manual(self, basic_3d_data):
        ...
        # New implementation
        k_new = Koopman(
            observables=TimeDelay(n_delays=n_delays), regressor=pDMD(svd_rank=rank)
        )
    ...
    def test_2d_timedelay_backward_compatible(self):
        ...
        k = Koopman(observables=TimeDelay(n_delays=3), regressor=pDMD(svd_rank=5))
        k.fit(data_2d)
...
class TestKoopmanObservables:
    ...
    def test_identity_3d(self, basic_3d_data):
        ...
        k = Koopman(observables=Identity(), regressor=pDMD(svd_rank=2))
        k.fit(basic_3d_data)
        assert k.A.shape == (2, 2)

    def test_polynomial_3d(self, basic_3d_data):
        """Test Polynomial observable with 3D data."""
        k = Koopman(observables=Polynomial(degree=2), regressor=pDMD(svd_rank=5))
        k.fit(basic_3d_data)
        assert k.A.shape == (5, 5)
...
class TestKoopmanVariableLength:
    ...
    def test_variable_length_sample_count(self, variable_length_data):
        ...
        k = Koopman(
            observables=TimeDelay(n_delays=n_delays), regressor=pDMD(svd_rank=5)
        )
        k.fit(variable_length_data)
...
def test_parametrized_timedelay(basic_3d_data, n_delays, rank):
    ...
    k = Koopman(observables=TimeDelay(n_delays=n_delays), regressor=pDMD(svd_rank=rank))
    k.fit(basic_3d_data)
