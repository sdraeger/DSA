...
def test_simdist_wasserstein_with_matrices(random_matrices):
    ...
    simdist = SimilarityTransformDist(
        iters=100, score_method="wasserstein", lr=0.01, device="cpu", verbose=False
    )
    ...
    assert score_identical < 1e-3, (
        f"Identical matrices should have near-zero score, got {score_identical}"
    )
...
def test_simdist_wasserstein_with_precomputed_eigenvalues(random_matrices):
    ...
    assert score_identical_eig < 1e-3, (
        f"Identical eigenvalues should have near-zero score, got {score_identical_eig}"
    )
...
def test_simdist_wasserstein_with_numpy_arrays(random_matrices):
    ...
    A_np = A.numpy()
    B_np = B.numpy()
    eigenvalues_A_np = np.linalg.eig(A_np)[
        0
...
def test_dsa_wasserstein_caching(random_data):
    ...
    assert np.all(diagonal_scores < 1e-3), (
        f"Diagonal should be near-zero, got {diagonal_scores}"
    )

    # Check that matrix is symmetric
    symmetry_diff = np.abs(scores - scores.T).max()
    assert symmetry_diff < 1e-6, (
        f"Score matrix should be symmetric, got max diff {symmetry_diff}"
    )

    # Verify the optimization actually cached the eigenvalues
    assert hasattr(dsa, "cached_compare_objects"), (
        "DSA should have cached_compare_objects"
    )
    assert len(dsa.cached_compare_objects) == 2, (
        "Should have 2 groups of cached objects"
    )
